...
app.get("/api/counsellors", async (req, res) => {
  try {
    const db = await connectDB();
    const result = await db.request().query(`
      SELECT HC_ID, HC_Name, HC_Sp_Field, HC_Profile_URL, HC_WD_Timing, HC_SD_Timing 
      FROM HC_Table
    `);
...
// Fetch a single counsellor by ID
app.get("/api/counsellor/:hc_id", async (req, res) => {
  try {
    const { hc_id } = req.params;
    const db = await connectDB();

    const result = await db.request()
      .input("HC_ID", sql.Int, hc_id)
      .query(`
        SELECT HC_ID, HC_Name, HC_Desc, HC_Sp_Field, HC_Profile_URL, HC_WD_Timing, HC_SD_Timing
        FROM HC_Table
        WHERE HC_ID = @HC_ID
      `);
